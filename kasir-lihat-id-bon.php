...
$query_items = "SELECT ti.*, m.menu, m.gambar FROM transaksi_items ti JOIN menu m ON ti.id_menu = m.id_menu WHERE ti.id_pesanan = $id_pesanan";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_query = "UPDATE transaksi_items SET status_pembayaran = '$status_pembayaran', tanggal = NOW() WHERE id_pesanan = $id_pesanan";
...
<head>
    ...
    <style>
        ...
        .back-button, .pdf-button, .buttonSimpan {
            width: 100%;
        ...
        .back-button:hover, .pdf-button:hover, .buttonSimpan:hover {
    ...
    <div class="receipt-container">
        <table>
            ...
            if ($result_items && mysqli_num_rows($result_items) > 0) {
                ...
                echo "<tr><td colspan='5'>Tidak ada item pesanan untuk ditampilkan.</td></tr>";
            }
            ?>
            <tr>
        ...
        <form method="POST" class="status-form">
            ...
            <select name="status_pembayaran">
                <option value="belum dibayar" <?php if ($status_pembayaran == 'belum dibayar') echo 'selected'; ?>>Belum Dibayar</option>
                <option value="dibayar" <?php if ($status_pembayaran == 'dibayar') echo 'selected'; ?>>Dibayar</option>
            </select>
            <button type="submit" class="buttonSimpan">Simpan</button>
        </form>

        <button class="back-button" onclick='window.location.href="view-kasir.php"'>Kembali</button>
